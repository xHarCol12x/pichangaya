...
export class AiToolsService {
  ...
  async getAvailability(date: string, venueId: string) {
    if (!date || !venueId)
      throw new BadRequestException('date and venueId are required');
    ...
    return {
      fecha: date,
      instruccion_para_ia:
        'Revisa las horas ocupadas. Si la hora pedida por el cliente NO está ocupada, significa que está libre y puedes ofrecerla. Devuelve el id de la cancha al reservar.',
      canchas: summary,
    };
  }
  ...
  async createBooking(data: {
    ...
    if (!tenantMember)
      throw new BadRequestException(
        'No se encontró un administrador para este negocio.',
      );
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (collision) {
        throw new BadRequestException(
          'La cancha ya no está disponible en ese horario.',
        );
      }
      ...
      return {
        ...
        mensaje_para_ia:
          'Reserva creada con éxito. Informa al cliente que el total es ' +
          price +
          ' y pásale el link de pago.',
