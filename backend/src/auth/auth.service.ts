  ...
  async login(user: any) {
    ...
    try {
      ...
      if (isActuallyActive && user.subscriptionEndsAt) {
        ...
        if (new Date(user.subscriptionEndsAt) <= now) {
          console.log(
            `[STABILITY-LOG] User subscription expired: ${user.email}`,
          );
          isActuallyActive = false;
        }
      }
      ...
      try {
        ...
        console.error(
          '[STABILITY-LOG] Error fetching plan permissions during login:',
          e,
        );
      }
      ...
      if (typeof featureOverrides === 'string') {
        try {
          featureOverrides = JSON.parse(featureOverrides);
          console.log(
            `[STABILITY-LOG] Parsed featureOverrides from string for: ${user.email}`,
          );
        } catch (e) {
          console.error(
            '[STABILITY-LOG] Failed to parse featureOverrides string:',
            e,
          );
          featureOverrides = {};
        }
