  ...
  async findOne(id: string, ownerId: string): Promise<Booking | null> {
    const booking = await this.prisma.booking.findFirst({
      where: {
        id,
        deletedAt: null,
        OR: [{ userId: ownerId }, { field: { venue: { ownerId } } }],
      },
      include: {
        user: true,
        field: { include: { venue: true } },
  ...
  async create(
    ...
    if (hasClientId) {
      ...
      if (!client) {
        throw new NotFoundException(
          'El cliente seleccionado no existe o no te pertenece.',
        );
      }
    }
    ...
    if (existing) {
      throw new ConflictException(
        'Esta cancha ya está reservada para ese horario.',
      );
    }
  ...
  async update(
    ...
    if (data.clientId !== undefined) {
      if (data.clientId && data.clientId !== '') {
        ...
        if (!client) {
          throw new NotFoundException(
            'El cliente especificado no te pertenece.',
          );
        }
      }
    }
