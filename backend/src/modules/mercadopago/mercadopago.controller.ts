...
export class MercadoPagoController {
    ...
    async createPreference(@Req() req, @Body('planName') planName: string, @Body('price') price: number) {
        return this.mpService.createPreference(req.user.userId, planName, price);
    }

    @UseGuards(JwtAuthGuard)
    @Post('create-subscription')
    async createSubscription(@Req() req, @Body('planName') planName: string, @Body('price') price: number) {
