...
export class MercadoPagoService {
  ...
  async createPreference(
    userId: string,
    planCode: string,
    interval: 'mensual' | 'anual' = 'mensual',
  ) {
    try {
      ...
      const price =
        interval === 'mensual' ? plan.priceMensual : plan.priceAnual;
  ...
  async handleWebhook(data: any, headers?: any) {
    ...
    if (
      (data.type === 'payment' || data.topic === 'payment') &&
      ...
      try {
        ...
        if (paymentInfo.status === 'approved') {
          const userId =
            paymentInfo.external_reference || paymentInfo.metadata?.user_id;
