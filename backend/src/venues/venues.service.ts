...
export class VenuesService {
    ...
    async create(tenantId: string, data: { name: string; address: string; description?: string }): Promise<Venue> {
    ...
    async update(id: string, tenantId: string, data: { name?: string; address?: string; description?: string }): Promise<Venue> {
        const existing = await this.prisma.venue.findFirst({ where: { id, tenantId, deletedAt: null } });
        if (!existing) {
